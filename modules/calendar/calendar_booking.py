# -*- coding: utf-8 -*-
"""
Online rezervační systém pro zákazníky
"""
...
class BookingRequestDialog(QDialog):
    ...
    def init_ui(self):
        ...
        info_layout.addRow("Typ služby:", self.lbl_service)

        self.lbl_date = QLabel("-")
        info_layout.addRow("Datum:", self.lbl_date)

        self.lbl_time = QLabel("-")
        info_layout.addRow("Čas:", self.lbl_time)
        ...
        customer_group = QGroupBox("Kontaktní údaje")
        customer_layout = QFormLayout(customer_group)
        customer_layout.setSpacing(10)

        self.lbl_customer_name = QLabel("-")
        customer_layout.addRow("Jméno:", self.lbl_customer_name)
        ...
        vehicle_layout.addRow("Popis problému:", self.lbl_description)
        ...
        if self.booking_id:
            ...
            self.btn_approve = QPushButton("✅ Schválit")
            ...
            self.btn_reject = QPushButton("❌ Zamítnout")
    ...
    def approve_booking(self):
        try:
            ...
            QMessageBox.information(
                self,
                "Schváleno",
                "Rezervace byla schválena a zapsána do kalendáře.\n"
                "Zákazníkovi bude odeslán potvrzovací email."
            )
            self.accept()
        except Exception as e:
            QMessageBox.critical(self, "Chyba", f"Chyba při schvalování: {e}")
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                QMessageBox.information(
                    self,
                    "Zamítnuto",
                    "Rezervace byla zamítnuta.\n"
                    "Zákazníkovi bude odeslán informační email."
                )
                self.accept()
            except Exception as e:
                QMessageBox.critical(self, "Chyba", f"Chyba při zamítání: {e}")

    def reschedule_booking(self):
        QMessageBox.information(
            self,
            "Změna termínu",
            "Otevře se dialog pro výběr nového termínu.\n"
            "Zákazník bude informován o změně."
        )
    ...
    def contact_customer(self):
        ...
        QMessageBox.information(
            ...
            f"Telefon: {phone}\n"
            f"Email: {email}\n\n"
            "Můžete kontaktovat zákazníka pro upřesnění detailů."
        )


class BookingSettingsDialog(QDialog):
    """Dialog pro nastavení rezervačního systému"""

    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Nastavení rezervací")
    ...
    def init_ui(self):
        ...
        services_group = QGroupBox("Povolené služby")
        ...
        timing_group = QGroupBox("Časová omezení")
        ...
        timing_layout.addRow("Minimální předstih:", self.spin_min_advance)

        self.spin_max_advance = QSpinBox()
        self.spin_max_advance.setRange(1, 365)
        self.spin_max_advance.setValue(30)
        self.spin_max_advance.setSuffix(" dní")
        timing_layout.addRow("Maximální předstih:", self.spin_max_advance)
    ...
    def load_settings(self):
        ...
        if settings:
            if dict(settings).get('allowed_services'):
                services = json.loads(settings['allowed_services'])
                for key, cb in self.service_checkboxes.items():
                    cb.setChecked(key in services)

            if dict(settings).get('min_advance_hours'):
                self.spin_min_advance.setValue(settings['min_advance_hours'])
            if dict(settings).get('max_advance_days'):
                self.spin_max_advance.setValue(settings['max_advance_days'])
            if dict(settings).get('slot_duration'):
                self.spin_slot_duration.setValue(settings['slot_duration'])

            self.chk_auto_approve.setChecked(bool(dict(settings).get('auto_approve', 0)))
            self.chk_email_notification.setChecked(bool(dict(settings).get('email_notification', 1)))
            self.chk_sms_notification.setChecked(bool(dict(settings).get('sms_notification', 0)))
...
class CalendarBooking(QWidget):
    """Widget pro správu online rezervací"""
    ...
    def init_ui(self):
        ...
        tabs = QTabWidget()
        tabs.addTab(self.create_pending_tab(), "⏳ Čekající na schválení")
        tabs.addTab(self.create_all_bookings_tab(), "📋 Všechny rezervace")
        tabs.addTab(self.create_available_slots_tab(), "🟢 Dostupné termíny")
        tabs.addTab(self.create_statistics_tab(), "📊 Statistiky")
    ...
    def create_top_panel(self):
        ...
        self.lbl_pending_count = QLabel("⏳ Čekající: 0")
        ...
        self.btn_generate_slots = QPushButton("🟢 Generovat volné termíny")
    ...
    def create_pending_tab(self):
        ...
        self.pending_table.setHorizontalHeaderLabels([
            "ID", "Datum", "Čas", "Zákazník", "Telefon", "Služba", "Vytvořeno", "Akce"
        ])
        ...
        self.btn_approve_all = QPushButton("✅ Schválit všechny")
    ...
    def create_all_bookings_tab(self):
        ...
        self.cmb_status_filter.addItem("Všechny", None)
        self.cmb_status_filter.addItem("⏳ Čekající", "pending")
        self.cmb_status_filter.addItem("✅ Schválené", "approved")
        self.cmb_status_filter.addItem("❌ Zamítnuté", "rejected")
        self.cmb_status_filter.addItem("✔️ Dokončené", "completed")
        self.cmb_status_filter.currentIndexChanged.connect(self.load_all_bookings)
        filter_layout.addWidget(self.cmb_status_filter)

        filter_layout.addWidget(QLabel("Období:"))
        ...
        self.all_bookings_table.setHorizontalHeaderLabels([
            "ID", "Datum", "Čas", "Zákazník", "Služba", "Stav", "Vytvořeno", "Akce"
        ])
    ...
    def create_available_slots_tab(self):
        ...
        header_layout.addWidget(QLabel("Zobrazit termíny pro:"))

        self.cmb_days_ahead = QComboBox()
        self.cmb_days_ahead.addItem("Příštích 7 dní", 7)
        self.cmb_days_ahead.addItem("Příštích 14 dní", 14)
        self.cmb_days_ahead.addItem("Příštích 30 dní", 30)
        ...
        self.slots_table.setHorizontalHeaderLabels(["Datum", "Den", "Čas", "Dostupnost", "Akce"])
    ...
    def create_statistics_tab(self):
        ...
        self.lbl_total_bookings = QLabel("Celkem rezervací: 0")
        ...
        charts_group = QGroupBox("Rezervace podle služeb (tento měsíc)")
    ...
    def update_pending_count(self):
        ...
        self.lbl_pending_count.setText(f"⏳ Čekající: {pending}")
    ...
    def load_pending_bookings(self):
        ...
        for row, booking in enumerate(bookings):
            ...
            btn_approve.setToolTip("Schválit")
            ...
            btn_reject.setToolTip("Zamítnout")
    ...
    def load_all_bookings(self):
        ...
        for row, booking in enumerate(bookings):
            ...
            btn_detail = QPushButton("👁️ Detail")
            btn_detail.clicked.connect(lambda checked, bid=booking['id']: self.show_booking_detail(bid))
    ...
    def load_available_slots(self):
            ...
            for start_h, end_h in work_hours:
                for hour in range(start_h, end_h):
                    ...
                    if time_str not in booked_set:
                        slots.append((check_date, days_cz[check_date.weekday()], time_str, "🟢 Volný"))
                    else:
                        slots.append((check_date, days_cz[check_date.weekday()], time_str, "🔴 Obsazený"))
        ...
        for row, (slot_date, day_name, time_str, status) in enumerate(slots):
            ...
            if "Volný" in status:
                btn_block = QPushButton("🚫 Blokovat")
    ...
    def load_statistics(self):
        ...
        self.lbl_total_bookings.setText(f"Celkem rezervací: {total['count'] if total else 0}")
        ...
        if total_count > 0:
            ...
            self.lbl_approved_rate.setText(f"Úspěšnost: {rate}%")
        else:
            self.lbl_approved_rate.setText("Úspěšnost: 0%")

        self.lbl_avg_response.setText("Průměrná odezva: 2 h")
        ...
        if popular:
            ...
            self.lbl_popular_service.setText(f"Nejčastější služba: {service_name}")
        else:
            self.lbl_popular_service.setText("Nejčastější služba: -")
        ...
        for stat in services_stats:
            ...
            item = QListWidgetItem(f"{service_name}: {stat['count']} rezervací")
    ...
    def quick_contact(self, phone):
        QMessageBox.information(self, "Kontakt", f"Telefon zákazníka: {phone}")

    def approve_all_pending(self):
        pending = db.fetch_all("SELECT id FROM calendar_bookings WHERE status = 'pending'")

        if not pending:
            QMessageBox.information(self, "Info", "Žádné čekající rezervace.")
            return

        reply = QMessageBox.question(
            self,
            "Potvrzení",
            f"Opravdu chcete schválit všech {len(pending)} čekajících rezervací?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
        )
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                QMessageBox.information(self, "Úspěch", f"Schváleno {len(pending)} rezervací.")
                self.refresh()
            except Exception as e:
                QMessageBox.critical(self, "Chyba", f"Chyba při hromadném schvalování: {e}")

    def block_slot(self, slot_date, time_str):
        reply = QMessageBox.question(
            self,
            "Blokování termínu",
            f"Chcete zablokovat termín {slot_date.strftime('%d.%m.%Y')} v {time_str}?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
        )
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            try:
                ...
                QMessageBox.information(self, "Úspěch", "Termín byl zablokován.")
                self.load_available_slots()
            except Exception as e:
                QMessageBox.critical(self, "Chyba", f"Chyba při blokování: {e}")

    def generate_slots(self):
        QMessageBox.information(
            self,
            "Generování termínů",
            "Volné termíny jsou generovány automaticky na základě pracovní doby servisu "
            "a dostupnosti mechaniků.\n\n"
            "Termíny jsou aktualizovány při každém obnovení obrazovky."
        )
