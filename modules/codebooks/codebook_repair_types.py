# -*- coding: utf-8 -*-
"""
Modul Číselníky - Typy oprav (PRODUKČNÍ VERZE)
"""
...
class RepairTypesWidget(QWidget):
    ...
    def init_ui(self):
        ...
        self.table.setHorizontalHeaderLabels([
            "ID", "Kód", "Název", "Kategorie", "Cena (Kč)", "Čas (h)", "Aktivní", "Akce"
        ])
    ...
    def create_top_panel(self):
        ...
        # Tlačítka
        add_btn = QPushButton("➕ Přidat typ opravy")
        ...
        self.search_input.setPlaceholderText("🔍 Vyhledat typ opravy...")
    ...
    def create_filter_panel(self):
        ...
        self.category_filter.addItem("Všechny kategorie", "all")
        self.category_filter.addItem("🔧 Servis", "servis")
        self.category_filter.addItem("🛠️ Oprava", "oprava")
        self.category_filter.addItem("🔍 Diagnostika", "diagnostika")
        self.category_filter.addItem("⚙️ Montáž", "montaz")
        self.category_filter.addItem("🎨 Lakování", "lakovani")
        self.category_filter.addItem("📋 Ostatní", "ostatni")
        ...
        self.sort_combo.addItem("Podle času", "time")
    ...
    def create_bottom_panel(self):
        ...
        self.count_label = QLabel("Celkem: 0 typů oprav")
    ...
    def display_data(self, data):
        ...
        for row, repair_type in enumerate(data):
            ...
            # Aktivní
            active_item = QTableWidgetItem("✅" if repair_type["active"] else "❌")
        ...
        self.count_label.setText(f"Celkem: {len(data)} typů oprav")

    def add_repair_type(self):
        """Přidání nového typu opravy"""
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            try:
                ...
                QMessageBox.information(self, "Úspěch", f"Typ opravy '{data['name']}' byl přidán.")
                self.load_data()
                ...
                if "UNIQUE constraint failed" in str(e):
                    ...
                    QMessageBox.critical(self, "Chyba", f"Nepodařilo se přidat typ opravy:\n{e}")
    ...
    def edit_repair_type(self, repair_type):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            try:
                ...
                QMessageBox.information(self, "Úspěch", "Typ opravy byl upraven.")
                self.load_data()
                self.data_changed.emit()

            except Exception as e:
                QMessageBox.critical(self, "Chyba", f"Nepodařilo se upravit typ opravy:\n{e}")

    def duplicate_repair_type(self, repair_type):
        """Duplikování typu opravy"""
        try:
            # Generovat nový kód
            ...
            QMessageBox.information(self, "Úspěch", f"Typ opravy byl zduplikován s kódem '{new_code}'.")
            self.load_data()
            self.data_changed.emit()

        except Exception as e:
            QMessageBox.critical(self, "Chyba", f"Nepodařilo se duplikovat typ opravy:\n{e}")
    ...
    def delete_repair_type(self, repair_type):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                QMessageBox.information(self, "Úspěch", "Typ opravy byl smazán.")
                self.load_data()
                self.data_changed.emit()

            except Exception as e:
                QMessageBox.critical(self, "Chyba", f"Nepodařilo se smazat typ opravy:\n{e}")
        ...
        try:
            ...
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                for row in reader:
                    ...
                    category = dict(row).get("category", "ostatni").strip()
                    default_price = float(dict(row).get("default_price", 0))
                    default_time = float(dict(row).get("default_time", 0))
            ...
            QMessageBox.information(
                self,
                "Import dokončen",
                f"Importováno: {imported} typů oprav\nPřeskočeno (duplicity): {skipped}"
            )
    ...
    def export_csv(self):
        ...
        try:
            ...
            QMessageBox.information(
                self,
                "Export dokončen",
                f"Exportováno {len(repair_types)} typů oprav do:\n{file_path}"
            )
    ...
    def reset_to_default(self):
        """Obnovení výchozích typů oprav"""
        reply = QMessageBox.question(
            self,
            "Obnovit výchozí typy oprav",
            "Opravdu chcete obnovit výchozí typy oprav?\n\n"
            "Budou přidány chybějící typy, existující zůstanou.",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
        )
        ...
        try:
            default_types = [
                # Servis
                ("SRV01", "Výměna oleje a filtru", "servis", 800, 1.0),
                ("SRV02", "Výměna vzduchového filtru", "servis", 400, 0.5),
                ("SRV03", "Výměna kabinového filtru", "servis", 500, 0.5),
                ("SRV04", "Výměna palivového filtru", "servis", 600, 0.75),
                ("SRV05", "Výměna brzdové kapaliny", "servis", 700, 1.0),
                ("SRV06", "Výměna chladicí kapaliny", "servis", 900, 1.5),
                ("SRV07", "Kontrola a doplnění kapalin", "servis", 300, 0.5),

                # Opravy
                ("OPR01", "Výměna brzdových destiček - přední", "oprava", 1500, 2.0),
                ("OPR02", "Výměna brzdových destiček - zadní", "oprava", 1200, 1.5),
                ("OPR03", "Výměna brzdových kotoučů", "oprava", 2500, 3.0),
                ("OPR04", "Výměna rozvodového řemene", "oprava", 5000, 6.0),
                ("OPR05", "Výměna rozvodového řetězu", "oprava", 8000, 8.0),
                ("OPR06", "Výměna spojky", "oprava", 6000, 7.0),
                ("OPR07", "Výměna tlumičů - přední", "oprava", 4000, 4.0),
                ("OPR08", "Výměna tlumičů - zadní", "oprava", 3500, 3.5),
                ("OPR09", "Výměna akumulátoru", "oprava", 500, 0.5),
                ("OPR10", "Výměna zapalovacích svíček", "oprava", 800, 1.0),

                # Diagnostika
                ("DIA01", "Diagnostika závady", "diagnostika", 600, 0.5),
                ("DIA02", "Kompletní diagnostika vozidla", "diagnostika", 1500, 2.0),
                ...
                # Montáž
                ("MON01", "Přezutí pneumatik (4 ks)", "montaz", 600, 1.0),
                ("MON02", "Vyvážení kol (4 ks)", "montaz", 400, 0.5),
                ("MON03", "Geometrie kol", "montaz", 1200, 1.5),
                ("MON04", "Montáž tažného zařízení", "montaz", 3000, 4.0),
                ("MON05", "Montáž střešního nosiče", "montaz", 800, 1.0),

                # Ostatní
                ("OST01", "Příprava na STK", "ostatni", 1500, 2.0),
                ("OST02", "Příprava na měření emisí", "ostatni", 800, 1.0),
                ("OST03", "Mytí motoru", "ostatni", 500, 0.5),
                ("OST04", "Čištění klimatizace", "ostatni", 800, 1.0),
                ("OST05", "Plnění klimatizace", "ostatni", 1200, 1.5),
            ]
            ...
            QMessageBox.information(
                self,
                "Dokončeno",
                f"Přidáno {added} výchozích typů oprav."
            )
            ...
            QMessageBox.critical(self, "Chyba", f"Nepodařilo se obnovit výchozí typy oprav:\n{e}")
...
class RepairTypeDialog(QDialog):
    """Dialog pro přidání/úpravu typu opravy"""

    def __init__(self, parent, repair_type=None):
        super().__init__(parent)
        self.repair_type = repair_type
        self.setWindowTitle("Upravit typ opravy" if repair_type else "Nový typ opravy")
    ...
    def init_ui(self):
        ...
        self.code_input.setPlaceholderText("Např: SRV01, OPR02...")
        self.code_input.setMaxLength(10)
        layout.addRow("Kód:", self.code_input)

        # Název
        self.name_input = QLineEdit()
        self.name_input.setPlaceholderText("Např: Výměna oleje a filtru")
        layout.addRow("Název:", self.name_input)

        # Kategorie
        self.category_combo = QComboBox()
        self.category_combo.addItem("🔧 Servis", "servis")
        self.category_combo.addItem("🛠️ Oprava", "oprava")
        self.category_combo.addItem("🔍 Diagnostika", "diagnostika")
        self.category_combo.addItem("⚙️ Montáž", "montaz")
        self.category_combo.addItem("🎨 Lakování", "lakovani")
        self.category_combo.addItem("📋 Ostatní", "ostatni")
        layout.addRow("Kategorie:", self.category_combo)

        # Cena a čas
        price_time_group = QGroupBox("Výchozí hodnoty")
        ...
        price_time_layout.addRow("Čas:", self.time_input)

        # Kalkulátor
        calc_layout = QHBoxLayout()
        calc_label = QLabel("Rychlý výpočet:")
        ...
        self.hourly_rate.setSuffix(" Kč/h")
        calc_layout.addWidget(self.hourly_rate)

        calc_btn = QPushButton("Vypočítat")
        ...
        self.description_input.setPlaceholderText("Detailní popis práce...")
    ...
    def calculate_price(self):
        """Výpočet ceny podle hodinové sazby"""
    ...
    def save(self):
        ...
        if not code:
            QMessageBox.warning(self, "Chyba", "Vyplňte kód typu opravy.")
            return

        if not name:
            QMessageBox.warning(self, "Chyba", "Vyplňte název typu opravy.")
            return
